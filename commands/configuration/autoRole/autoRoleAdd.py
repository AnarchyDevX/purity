...
class autoRoleAdd(commands.Cog):
    ...
    async def autoroleadd(self, interaction: discord.Interaction, role: discord.Role):
        ...
        if role.id in roleList:
            return await err_embed(
                interaction,
                title="Role déja configurer",
                description=f"Le role {role.mention} est déja présent dans la liste des rôles ajoutés a l'arrivée"
            )        
        ...
        embed: embedBuilder = embedBuilder(
            ...
            description=f"*Le role {role.mention} sera maintenant ajouté a chaque membre qui rejoindrai le serveur*",
            footer=footer(),
            color=embed_color()
        )
...
async def setup(bot: commands.Bot) -> None:
    await bot.add_cog(autoRoleAdd(bot))