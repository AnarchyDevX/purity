...
class logsBan(commands.Cog):
    def __init__(self, bot: commands.Bot) -> None:
        self.bot: commands.Bot = bot
    
    @app_commands.command(name="logs-moderation", description="Activer / Désactiver les logs de moderation")
    ...
    async def logsBan(self, interaction: discord.Interaction, alive: str, channel: discord.TextChannel = None) -> None:
        check: bool = await check_perms(interaction, 2)
        await logs("logs-moderation", 1, interaction)
        if check == False:
            return
        ...
        if alive == "no":
            ...
            embed: embedBuilder = embedBuilder(
                ...
                description=f"*Les logs de modération ont été désactivées*",
                color=embed_color(),
                footer=footer()
            )    
            ...
            embed: embedBuilder = embedBuilder(
                title="`✅`・Logs de modération",
                description=f"*Les logs de modération ont été activées dans le salon {channel.mention}.*",
                color=embed_color(),
                footer=footer()
            )    
            return await interaction.response.send_message(embed=embed)
