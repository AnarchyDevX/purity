...
class voiceUndeafed(commands.Cog):
    ...
    async def on_voice_state_update(self, member: discord.Member, before: discord.VoiceState, after: discord.VoiceState):
        ...
        if logsChannel:
            if before.deaf == True and after.deaf == False:
                embed = embedBuilder(
                    description=f"```[{time_now()}] - Voice | Sourdine désactivée```",
                    color=embed_color(),
                    footer=footer(),
                    fields={
                ...
                async for entry in member.guild.audit_logs(limit=1, action=discord.AuditLogAction.member_update):
                    if entry.target.id == member.id and not entry.changes.after.deaf:
                        embed.add_field(
...
async def setup(bot):
    await bot.add_cog(voiceUndeafed(bot))