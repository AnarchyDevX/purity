...
class voiceSelfUncam(commands.Cog):
    ...
    async def on_voice_state_update(self, member: discord.Member, before: discord.VoiceState, after: discord.VoiceState):
        ...
        if logsChannel:
            if before.self_video == True and after.self_video == False:
                embed = embedBuilder(
                    description=f"```[{time_now()}] - Voice | Caméra Désactivée```",
                    color=embed_color(),
                    footer=footer(),
                    fields={
...
async def setup(bot):
    await bot.add_cog(voiceSelfUncam(bot))