...
async def generate_ticket_transcript_txt(channel: discord.TextChannel) -> str:
    """Génère un transcript en format texte simple"""
...
async def send_ticket_transcript(channel: discord.TextChannel, transcript: str, logs_channel: Optional[discord.TextChannel], vps_url: Optional[str] = None, use_txt: bool = True) -> Optional[discord.Message]:
    """Envoie le transcript dans le salon de logs"""
    
    if not logs_channel:
        return None
    
    try:
        # Si on veut utiliser le format TXT
        if use_txt:
            import io
        ...
        # Ancien système avec VPS (pour référence future)
        transcript_url = None
        
        # Si une URL VPS est configurée, uploader le fichier
        if vps_url:
        ...
        if transcript_url:
            ...
            # Fallback: envoyer le fichier directement si pas de VPS ou erreur
            import io
            from discord import File
